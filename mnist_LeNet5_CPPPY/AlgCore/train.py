...
def train(model_path_b, steps, rate_base, rate_decay):
    ...
    # 定义输入输出占位
    x = tf.compat.v1.placeholder(tf.float32, [BATCH_SIZE, forward.IMAGE_SZIE, forward.IMAGE_SZIE, forward.NUM_CHANNELS], name='x-input')
    ...
    var_averages_op = var_averages.apply(tf.compat.v1.trainable_variables())
    ...
    loss = cross_entropy_mean + tf.compat.v1.add_n(tf.get_collection('losses'))
    ...
    """
    learning_rate = tf.compat.v1.train.exponential_decay(rate_base, global_step,
